...
\begin{figure}[H]
  \centering
  \shorthandoff{<>}% desactivar shorthands '<' y '>' de babel en español
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
    ...
    \node[box,right=22mm of esclavo] (maestro) {MAESTRO\\(MKR WAN)\\LoRa TX};
    \node[box,below=15mm of maestro] (gateway) {GATEWAY\\(MKR WAN)\\LoRa RX/TX};
    \node[box,left=22mm of gateway] (raspi) {RASPBERRY PI\\MQTT Broker};
    \node[box,right=22mm of gateway] (actuador) {ACTUADOR\\(Arduino UNO)\\LED/Servo};
    \node[box,below=15mm of raspi] (apps) {APLICACIONES\\Dashboard};

    % Connections (solid = serial, dashed = LoRa)
    \draw[serial] (esclavo) -- node[midway,above,lbl]{Serial} (maestro);
    \draw[lora] (maestro.south) .. controls +(-2mm,-8mm) and +(0,8mm) .. node[midway,right=1mm,lbl]{LoRa} (gateway.north);
    ...
    % Legend
    \begin{scope}[shift={(0,-45mm)}]
      \node[anchor=north west, font=\small] at (-50mm,0) {\textbf{Leyenda:}};
      \draw[serial] (-25mm,-4mm) -- ++(10mm,0) node[right=2mm, font=\tiny] {Serial};
      \draw[lora] (5mm,-4mm) -- ++(10mm,0) node[right=2mm, font=\tiny] {LoRa (dashed)};
    \end{scope}

    \end{tikzpicture}%
  }%
  \shorthandon{<>}% reactivar shorthands
  \caption{Diagrama de arquitectura del sistema. Líneas continuas: comunicación serial/MQTT. Líneas discontinuas: LoRa inalámbrico.}
