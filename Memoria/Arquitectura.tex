...
El sistema sigue una arquitectura distribuida de tipo \emph{sensor-gateway-actuador} con integración en la nube a través de MQTT. Los componentes principales son:

\begin{enumerate}
  \item \textbf{Nodo de sensores}: compuesto por un esclavo (Arduino UNO) y un maestro (MKR WAN 1310).
  \item \textbf{Gateway LoRa}: MKR WAN 1310 conectado a la Raspberry Pi.
  \item \textbf{Raspberry Pi}: ejecuta el broker MQTT y el puente serie-MQTT.
  \item \textbf{Nodo actuador}: Arduino UNO con módulo LoRa externo.
  \item \textbf{Aplicaciones cliente}: dashboard web, Node-RED, Home Assistant.
\end{enumerate}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    node distance=12mm,
    box/.style={draw, rounded corners, minimum width=28mm, minimum height=10mm, align=center, fill=blue!10},
    data/.style={draw, rounded corners, minimum width=24mm, minimum height=8mm, align=center, fill=green!10},
    arrow/.style={-{Stealth}, thick}
  ]
    % Nodos
    \node[box] (sensor) {Sensores\\(LDR, SRF)};
    \node[box, right=of sensor] (esclavo) {Esclavo};
    \node[box, right=of esclavo] (maestro) {Maestro};
    \node[box, right=of maestro] (gateway) {Gateway};
    \node[box, right=of gateway] (raspi) {Raspberry Pi};
    ...
    \draw[arrow, dashed] (maestro) -- node[above, font=\scriptsize]{LoRa} (gateway);
    \draw[arrow] (gateway) -- node[above, font=\scriptsize]{Serial} (raspi);
  \end{tikzpicture}
  \caption{Flujo de datos desde sensores hasta la Raspberry Pi.}
...
\begin{enumerate}
  ...
  \item El actuador ejecuta la acción y responde con ACK.
