...
\begin{itemize}
  \item \textbf{Arduino UNO}: microcontrolador principal que ejecuta la lógica de control.
  \item \textbf{Módulo LoRa SX1276}: receptor de comunicaciones inalámbricas a 868~MHz.
...
\begin{table}[H]
\centering
\caption{Conexiones del módulo LoRa SX1276 al Arduino UNO.}
\label{tab:conexiones-lora-actuador}
\begin{tabular}{ll}
\toprule
\textbf{Pin LoRa} & \textbf{Pin Arduino} \\
\midrule
NSS  & Pin 10 \\
MOSI & Pin 11 \\
MISO & Pin 12 \\
...
La dirección LoRa asignada al actuador es \texttt{0x06}, mientras que solo acepta mensajes provenientes del gateway (\texttt{0x05}). Esto proporciona una capa básica de seguridad al filtrar emisores no autorizados.
...
Para evitar que un mismo comando se ejecute múltiples veces (por ejemplo, debido a retransmisiones del gateway), el actuador mantiene el identificador del último mensaje procesado. Si recibe un paquete con el mismo \texttt{msgId}, responde con ACK pero no vuelve a ejecutar la acción:
